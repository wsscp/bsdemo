...
@Service
public class PlanScheduleTask extends BaseSimpleTask {
	...
	private static Date calculatorTime = null;
	...
	public void process(JobParams params) {
		...
		try {
			CustomerOrderItem customerOrderItem = customerOrderItemService.getLatestOrder();
			if(customerOrderItem == null || (customerOrderItem != null && calculatorTime != null && calculatorTime.compareTo(customerOrderItem.getCreateTime()) == 0)){
				return;
			}
			if(customerOrderItem != null){
				calculatorTime = customerOrderItem.getCreateTime();
			}
