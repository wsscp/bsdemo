 ...
 * 工序选择生成生产单
 */
Ext.define('bsmes.view.ShowProcessWorkOrderHTWindow', {
	extend : 'Ext.window.Window',
	alias : 'widget.showProcessWorkOrderHTWindow',
	...
	orderGrid:{
	    ...
	    columns : [{dataIndex : 'CONTRACTNO',text : '合同号', renderer : function(value, record){
			...
			{dataIndex : 'TASKLENGTH', text : '投产长度', renderer : function(value, record){
			...
			{dataIndex : 'CONTRACTLENGTH',text : '交货长度', renderer : function(value, record){
				var json = Ext.decode(record.OUTATTRDESC);
			...
			{dataIndex : 'PRODUCTTYPE',text : '电压', renderer : function(value, record){
				return value.substring(value.lastIndexOf('-') + 1);
			}}, 
			{dataIndex : 'OUTATTRDESC',text : '材料', renderer : function(value, record){
				var json = Ext.decode(value);
				return (typeof(json.material) == "undefined" ? '' : json.material);
			}}, 
			{dataIndex : 'COLOR',text : '颜色字码'},
			{dataIndex : 'OUTATTRDESC',text : '模芯模套', renderer : function(value, record){
				var json = Ext.decode(value);
				return (typeof(json.moldCoreSleeve) == "undefined" ? '' : json.moldCoreSleeve);
			}}, 
			{dataIndex : 'OUTATTRDESC',text : '标称厚度', renderer : function(value, record){
				var json = Ext.decode(value);
				return (typeof(json.standardPly) == "undefined" ? '' : json.standardPly);
			}}, 
			{dataIndex : 'OUTATTRDESC',text : '指导厚度', renderer : function(value, record){
				var json = Ext.decode(value);
				return (typeof(json.guidePly) == "undefined" ? '' : json.guidePly);
			}}, 
			{dataIndex : 'OUTATTRDESC',text : '标准外径', renderer : function(value, record){
				var json = Ext.decode(value);
				return (typeof(json.outsideValue) == "undefined" ? '' : json.outsideValue);
			}}, 
			{dataIndex : 'OUTATTRDESC',text : '最大外径', renderer : function(value, record){
				var json = Ext.decode(value);
				return (typeof(json.outsideMaxValue) == "undefined" ? '' : json.outsideMaxValue);
			}}, 
			{dataIndex : 'WIRECOIL',text : '收盘要求'},
			{dataIndex : 'OUTMATDESC',text : '印字要求', maxWidth : (document.body.scrollWidth/3)
		}]
	}, // 云母绕包的订单列表
	inGrid:{
		id : 'inTable',
	...
	initComponent : function() {
		...
		Ext.apply(me, {
			...
			title : '<div style="float:left;">' + me.title + '</div><span style="font-size: 16px;margin-left: -120px;">宝 胜 特 种 电 缆 有 限 公 司 护 套 工 序 生 产 单</span>',
