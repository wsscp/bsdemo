Ext.define('bsmes.controller.CraftsSLController', {
			extend : 'Oit.app.controller.GridController',
			view : 'craftsSLList', // 工艺列表
			// 质量参数新增弹出窗
			// 质量参数修改弹出窗
			views : ['CraftsSLList', 'InstanceProcessWindow'],
			stores : ['CraftsSLStore', 'ProcessInOutStore', 'ProcessMatPropStore', 'ProcessQcStore'],
			...
			showProcessWindow : function() {
				...
				if (selection && selection.length == 1) {
					...
					Ext.Ajax.request({
						url : 'process/getByCraftsId?craftsId=' + record.get('id'),
						success : function(response) {
							...
							Ext.Array.each(result, function(record, i) {
								...
								processArray.push({
									text : record.seq + '-' + record.processName + '[' + record.processCode + ']',
									id : record.id,
									leaf : true
								});
							});
