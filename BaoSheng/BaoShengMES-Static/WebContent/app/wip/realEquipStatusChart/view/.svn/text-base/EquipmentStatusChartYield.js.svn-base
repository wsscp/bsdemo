Ext.define("bsmes.view.EquipmentStatusChartYield", {
			extend : 'Ext.panel.Panel',
			alias : 'widget.equipmentStatusChartYield',
			title : '设备产量分析',
			chartPanelId : '', // 定义一个随机id
			yieldHighChar : null, // 线图
			...
			search : function() {
				...
				// 1、刷新线图
				me.freshYieldHighChar(startDate, endDate, type.type);
			},

			// 刷新加工时间：线图
			freshYieldHighChar : function(startTime, endTime, type) {
				var me = this;
				Ext.Ajax.request({
							url : 'realEquipStatusChart/equipHistoryYield',
							method : 'GET',
							params : {
							...
							success : function(response) {
								...
								if (me.yieldHighChar) {
									if (result.length > 0)
										me.yieldHighChar.series[0].setData(result[0].data);
								} else {
									var data = {
										name : '设备产量',
										data : []
									...
									me.createYieldHighChar(result[0]);
								}
							}
						});
			},

			createYieldHighChar : function(seriesData) {
				var me = this;
				me.yieldHighChar = $('#' + me.chartPanelId).highcharts({
							title : {
								text : '设备产量分析',
								x : -20
								// center
