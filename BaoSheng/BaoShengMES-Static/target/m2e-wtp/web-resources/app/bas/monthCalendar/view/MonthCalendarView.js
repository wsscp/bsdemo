 ...
 * @xtype calendarpanel
 */
Ext.define('bsmes.view.MonthCalendarView', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.calendarpanel',
    store : 'MonthCalendarStore',
    forceFit:false,
    defaults: {autoScroll: true}, 
    bodyStyle: 'min-height: 630px;',
    requires: [
    ...
    listeners: {
      	'dayclick': {
      		fn: function(vw, dt, ad, el){
      			...
      			var workMonth = year + '' + month;
      			var date = dt.getDate();
      			if(document.getElementById(el.id).className == "ext-cal-day ext-cal-day-today"){
      				Ext.Msg.show({
         			     ...
         			     fn: function(btn, txt){
         			    	 	if(btn=="yes"){
         			    	 		document.getElementById(el.id).className = "ext-cal-day";
         		        	  		Ext.Ajax.request({
         		        	        	method:'GET',
         		        	        	async: false,
         		        	            url: 'monthCalendar/updateDate',
         		        	            params: {
         		        	            	workMonth: workMonth,
         		        	            	day: date
         		        	            },
...
//         		        	            	console.log(response);
         				                ...
         				                failure : function(response) {
         				                	...
         				                	Ext.Msg.alert(Oit.msg.ERROR, "系统处理异常：" + response.message);
         								}
         		        	  		});
         			    	 	}
         				 	}
      					});
	        	  	}else if(document.getElementById(el.id).className == "ext-cal-day"){
	        	  		Ext.Msg.show({
	         			     title: Oit.msg.PROMPT,
	         			     msg: '确定更改?',
	         			     buttons: Ext.Msg.YESNO,
	         			     ...
	         			     fn: function(btn, txt){
	         			    	 	if(btn=="yes"){
	         			    	 		document.getElementById(el.id).className = "ext-cal-day ext-cal-day-today"
	         			    	  			Ext.Ajax.request({
	         			        	        	method:'GET',
	         			        	        	async: false,
	         			        	            url: 'monthCalendar/updateDate',
...
//	         			        	            	console.log(response);
    ...
    initComponent: function() {
            ...
            items: [{
                ...
                listeners: {
                    select: function(value){
                    	var tarYear = value.lastValue.getYear()+1900;
                    	var tarMonth = value.lastValue.getMonth();
                    	document.getElementById(me.id + '-tb-month-'+'btnInnerEl').innerHTML = tarYear+" 年 "+(tarMonth+1)+" 月";
                    	var preYear = me.startDate.getYear()+1900;
                    	var preMonth = me.startDate.getMonth();
                    	var months = (tarYear-preYear)*12 + (tarMonth-preMonth);
                    	me.startDate = me.layout.activeItem.changeMonth(months);
                    }
                }
            },"<span style='color:red;'>&nbsp;&nbsp;&nbsp;绿色为休息日,浅蓝色为工作日!</span>"]
        };
        ...
        if (this.showMonthView || this.viewCount == 0) {
        	var tarYear = new Date().getYear()+1900;
        	var tarMonth = new Date().getMonth()+1;
            this.tbar.items.push({
                id: this.id + '-tb-month',
                text: tarYear+" 年 "+(tarMonth)+" 月",
                scope: this,
                toggleGroup: 'tb-views'
            });
            this.viewCount++;
