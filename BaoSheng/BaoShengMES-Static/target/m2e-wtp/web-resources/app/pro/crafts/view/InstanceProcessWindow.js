...
Ext.define('bsmes.view.InstanceProcessWindow', {
	...
	showPropWin : function() {
		...
		if (selection && selection.length == 1) {
			...
			var win = me.getMatPropWin(record.get('matCode')); // processInOut的ID
			if (win)
				win.show();
	...
	getMatPropWin : function(matCode) {
		var me = this;
	...
	showAddWin : function(type, button) {
		var me = this;
		var win = null;
					...
					items : [{
						...
						items : [{
							...
							store : Ext.create('Ext.data.Store', {
										proxy : {
											type : 'rest',
											url : '../pla/materialRequirementPlan/getDescByMatName?matName='
										},
										autoLoad : true,
										fields : [{
													name : 'matCode',
							...
							listeners : {
								...
								beforequery : function(e) {
									...
									if (!e.forceAll) {
										var value = e.query;
										if (value != null && value != '') {
											combo.store.getProxy().url = '../pla/materialRequirementPlan/getDescByMatName?matName='
													+ value
											combo.store.load();
										} else {
